...
class TestFieldNormalization:
    ...
    def test_normalize_parses_user_agents(
        self, sample_upload_with_raw_data, test_db_path
    ):
        ...
        with DatabaseSession(test_db_path, use_dict_factory=True) as conn:
            ...
            for row in rows:
                ...
                if attrs.get("user_agent_original") or attrs.get("user_agent_os_full"):
                    ...
                    if (
                        "user_agent_os_name" in attrs
                        or "user_agent_os_type" in attrs
                        or "user_agent_device_model_name" in attrs
                        or "user_agent_client_name" in attrs
                    ):
                        rows_with_parsed += 1

            # If we had rows with UA data, verify they were parsed
            if rows_with_ua > 0:
                assert rows_with_parsed > 0, (
                    f"Had {rows_with_ua} rows with UA data but only {rows_with_parsed} were parsed"
                )

    def test_normalize_infers_device_manufacturer(
        self, sample_upload_with_raw_data, test_db_path
    ):
        ...
        with DatabaseSession(test_db_path, use_dict_factory=True) as conn:
            ...
            rows_with_ua = [
                r
                for r in rows
                if json.loads(r["attributes"]).get("user_agent_original")
                or json.loads(r["attributes"]).get("user_agent_os_full")
            ]
    ...
    def test_normalize_caches_user_agents(
        self, sample_upload_with_raw_data, test_db_path
    ):
        ...
        if records_normalized > 1:
            assert (
                unique_uas_parsed < records_normalized
                or unique_uas_parsed == records_normalized
    ...
    def test_normalize_with_hardcoded_user_agent_original(self, test_db_path):
        ...
        with DatabaseSession(test_db_path, use_dict_factory=True) as conn:
            rows = conn.execute(
                "SELECT attributes FROM devices_raw WHERE upload_id = ?", (upload_id,)
            ).fetchall()
            ...
            assert os_names == {"iOS", "Android", "Windows"}, (
                f"Expected iOS, Android, Windows but got {os_names}"
            )
            assert os_types == {"ios", "android", "windows"}, (
                f"Expected ios, android, windows but got {os_types}"
            )
            # Device models should be populated for at least some rows
            assert len(device_models) > 0, (
                f"Should have parsed device models but got none"
            )
    ...
    def test_normalize_with_hardcoded_user_agent_os_full(self, test_db_path):
        ...
        assert result["status"] == "success"
        assert result["records_normalized"] == 3
        assert result["unique_uas_parsed"] == 3, (
            "Should have parsed 3 unique OS strings"
        )

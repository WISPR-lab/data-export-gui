...
def test_json_parse(test_case):
    ...
    result = JSONParser.parse(content, test_case.filename, test_case.schema)
    validate_results(test_case, result.events, result.states, result.errors)


@pytest.mark.format("jsonl")
def test_jsonl_parse(test_case):
    content = test_case.read_content()
    result = JSONLParser.parse(content, test_case.filename, test_case.schema)
    validate_results(test_case, result.events, result.states, result.errors)
...
def test_json_label_values_parse(test_case):
    content = test_case.read_content()
    result = JSONLabelValuesParser.parse(content, test_case.filename, test_case.schema)
    validate_results(test_case, result.events, result.states, result.errors)

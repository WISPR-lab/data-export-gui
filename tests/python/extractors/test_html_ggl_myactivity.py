...
class TestHTMLMyActivityParserBasic:
    ...
    def test_parser_extracts_records_from_search_activity(self):
        ...
        assert len(records) > 0, (
            "Should extract at least one record from Search activity"
        )
...
class TestHTMLMyActivityParserFields:
    ...
    def test_record_contains_platform_field(self):
        ...
        for record in records:
            assert "Platform" in record, (
                f"Record missing Platform field: {record.keys()}"
            )
            assert record["Platform"] == "Search", (
                "Platform should be 'Search' for search activity"
            )
    ...
    def test_record_contains_timestamp_field(self):
        ...
        for record in records:
            ...
            assert any(char.isdigit() for char in record["Timestamp"]), (
                "Timestamp should contain digits"
            )
...
class TestHTMLMyActivityParserURLExtraction:
    ...
    def test_url_extracted_from_search_query(self):
        ...
        assert len(urls_found) > 0, (
            "Should extract at least one URL from search activity"
        )

        # URLs should start with http
        for record in urls_found:
            assert record["URL"].startswith(("http://", "https://")), (
                f"Invalid URL: {record['URL']}"
            )
...
class TestHTMLMyActivityParserLocationExtraction:
    ...
    def test_location_fields_extracted_when_present(self):
        ...
        locations_found = [
            r for r in records if any(key in r for key in ["Locations", "Location.URL"])
        ]
    ...
    def test_location_url_extracted_for_maps(self):
        ...
        for record in location_urls:
            ...
            assert url.startswith(("http://", "https://")), (
                f"Location URL should be valid: {url}"
            )
    ...
    def test_no_duplicate_keys_in_record(self):
        ...
        for record in records:
            ...
            assert len(keys) == len(unique_keys), (
                "Records should not have duplicate keys"
            )

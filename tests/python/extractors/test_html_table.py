...
class TestHTMLTableParserChangeHistory:
    ...
    def test_change_history_has_multiple_change_types(self):
        ...
        assert any(
            ct in change_types
            for ct in ["PASSWORD", "RECOVERY_EMAIL_VERIFIED", "SERVICE_ADDED"]
        )
    ...
    def test_change_history_timestamps_valid(self):
        ...
        for record in result:
            ...
            assert timestamp and ("Z" in timestamp or "-" in timestamp), (
            ...
            for table_name, records in result.items():
                if isinstance(records, list) and len(records) > 0:
                    ...
                    if "Activity Type" in first_record:
                        activity_types = set(
                            r.get("Activity Type", "") for r in records
                        )
                        assert any(
                            at in ["Login", "Logout"] for at in activity_types
    ...
    def test_subscriber_info_returns_multiple_tables(self):
        ...
        if isinstance(result, dict):
            ...
            for table_name in result.keys():
                assert table_name.startswith("table_") or isinstance(
    ...
    def test_all_records_are_dicts(self):
        ...
        for record in result:
            assert isinstance(record, dict), (
                f"Record should be dict, got {type(record)}"
            )
    ...
    def test_empty_cells_filled_with_empty_string(self):
        ...
        for record in result:
            for key, value in record.items():
                ...
                assert value == value or isinstance(value, str), (
                    f"Value should not be NaN: {value}"
                )
        ...
        for record in result:
            for key, value in record.items():
                ...
                assert isinstance(value, str), (
                    f"Value should be string, got {type(value)}: {value}"
                )
...
class TestHTMLTableParserFieldNames:
    ...
    def test_headers_become_field_names(self):
        ...
        assert field_names == expected_headers, (
            f"Fields {field_names} don't match headers {expected_headers}"
        )

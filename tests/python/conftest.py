...
def validate_results(test_case, records: list):
    ...
    if not expected:
        assert len(records) > 0, (
            f"No records parsed from {test_case.filename} and no 'expected' block provided"
        )
    ...
    if raw_keys:
        ...
        assert not missing, (
            f"Expected raw field(s) {missing} in first record. Got: {actual_keys}"
        )


def pytest_configure(config):
    config.addinivalue_line(
        "markers", "format(type): parser format to test (json, csv, etc.)"
    )
...
def inject_test_config(test_db_path):
    ...
    if hasattr(builtins, "TEMP_ZIP_DATA_STORAGE") and os.path.exists(
        builtins.TEMP_ZIP_DATA_STORAGE
    ):
...
def facebook_zip_path():
    ...
    path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "zip_data", "facebook.zip"
    )
...
def instagram_zip_path():
    ...
    path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "zip_data", "instagram.zip"
    )
    if not os.path.exists(path):
                ...
                for filename in filenames:
                    if filename.endswith(".json"):
                        ...
                        if manifest_file_id:
                            ...
                            from python_core.extractors.json_label_values import (
                            ...
                            try:
                                records = (
                                    parser.extract(content, parser_cfg)
                                    if parser
                                    else []
                                )
                            except Exception:

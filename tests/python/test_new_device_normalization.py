...
def mock_device_lookup():
    ...
    mock_lookup = {
        ...
        'VARIANT_SUFFIXES': {'pro', 'max', 'plus', 'ultra', 'lite', 'mini', 'xl', 's', 'se'},
...
class TestDecomposeOsVersion:
    ...
    @pytest.mark.parametrize('raw_version,expected_version,expected_name', [
        ('iOS 15.7', '15.7', 'iOS'),
    ...
    def test_decompose_os_version_valid_versions(self, raw_version, expected_version, expected_name):
        """Should correctly extract version number and OS name."""
        version, name = device._decompose_os_version(raw_version)
        assert version == expected_version
        assert name == expected_name

    @pytest.mark.parametrize('raw_version', [
        '',
        None,
        '   ',
    ])
...
class TestCompositeClientName:
    ...
    def test_composite_client_name_empty_primary(self):
        ...
        attrs = {'user_agent_client_name': '', 'user_agent_secondary_client_name': 'WebView'}
        result = device._composite_client_name(attrs)
        assert result is None

    def test_composite_client_name_empty_secondary_ignored(self):
        """Should ignore empty secondary client name."""
        attrs = {'user_agent_client_name': 'Safari', 'user_agent_secondary_client_name': ''}
        result = device._composite_client_name(attrs)
...
class TestNormalizeDeviceFields:
    ...
    def test_normalize_device_fields_manufacturer_alias_resolution(self, mock_device_lookup):
        """Should resolve manufacturer aliases."""
        attrs = {'device_manufacturer': 'cupertino'}
        result = device.normalize_device_fields(attrs)
        assert result['norm__manufacturer'] == 'apple'

    def test_normalize_device_fields_explicit_manufacturer_preferred(self, mock_device_lookup):
        """Explicit manufacturer should be preferred over inference."""
        attrs = {
            'device_model_name': 'Galaxy S20',
            'device_manufacturer': 'Samsung'
        }
        result = device.normalize_device_fields(attrs)
        assert result['norm__manufacturer'] == 'samsung'

    def test_normalize_device_fields_os_name_inference_from_version(self, mock_device_lookup):
    ...
    @pytest.mark.parametrize('input_attrs,expected_norm_keys', [
        (
            {'device_model_identifier': 'iPhone10,6', 'device_model_name': 'iPhone X'},
    ...
    def test_normalize_device_fields_parametrized(self, mock_device_lookup, input_attrs, expected_norm_keys):
        ...
        assert actual_norm_keys == expected_norm_keys, f"Expected {expected_norm_keys} but got {actual_norm_keys}"
...
class TestSemanticMap:
    ...
    def test_map_extracts_atomic_devices_from_facebook_zip(
        ...
        assert raw_data_count > 0, (
            "Sample data fixture should load at least one raw_data row"
        )
        ...
        with DatabaseSession(test_db_path, use_dict_factory=True) as conn:
            ...
            assert atomic_devices["count"] > 0, (
                f"Expected devices_raw rows, got {atomic_devices['count']}"
            )

    def test_map_populates_attributes_field(
        self, sample_upload_with_raw_data, test_db_path
    ):
        ...
        with DatabaseSession(test_db_path, use_dict_factory=True) as conn:
            ...
            for row in rows:
                ...
                assert isinstance(attrs, dict), (
                    f"Attributes should be dict, got {type(attrs)}"
                )

...
export default {
  ...
  methods: {
    getSuggestedQuery() {
      ...
      BrowserDB.llmRequest(this.sketch.id, 'nl2q', formData)
        .then((response) => {
    ...
    getQuestionTemplates() {
      this.isLoading = true
      BrowserDB.getQuestionTemplates()
        .then((response) => {
    ...
    getSketchQuestions() {
      this.isLoading = true
      return BrowserDB.getOrphanQuestions(this.sketch.id)
        .then((response) => {
    ...
    createQuestion(template = null) {
      ...
      BrowserDB.createQuestion(this.sketch.id, null, null, questionText, templateId)
        .then((response) => {
    ...
    getSearchHistory() {
      BrowserDB.getSearchHistory(this.sketch.id, 5, this.activeQuestion.id)
        .then((response) => {
    ...
    refreshActiveQuestion() {
      BrowserDB.getQuestion(this.sketch.id, this.activeQuestion.id)
        .then((response) => {
    ...
    createConclusion() {
      BrowserDB.createQuestionConclusion(this.sketch.id, this.activeQuestion.id, this.conclusionText)
        .then((response) => {
          this.conclusionText = ''
    ...
    deleteConclusion(conclusion) {
      BrowserDB.deleteQuestionConclusion(this.sketch.id, this.activeQuestion.id, conclusion.id)
        .then((response) => {
          this.refreshActiveQuestion()
          this.getSketchQuestions()
        })
    ...
    fetchQuestion(questionId) {
      ...
      return new Promise((resolve, reject) => {
        BrowserDB.getQuestion(this.sketch.id, questionId)
          .then((response) => {
    ...
    if (this.questionId) {
      ...
      if (!questionUUID) {
        ...
        if (Object.keys(context).length) {
          this.isLoading = true
          BrowserDB.getQuestion(this.sketch.id, context.questionId)
            .then((response) => {

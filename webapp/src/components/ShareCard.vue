...
export default {
  ...
  methods: {
    grantAccess: function () {
      ...
      BrowserDB.grantSketchAccess(this.sketch.id, usersToAdd, groupsToAdd)
        .then((response) => {
    ...
    revokeAccess: function (userName, groupName) {
      ...
      BrowserDB.revokeSketchAccess(this.sketch.id, userToRemove, groupToRemove)
        .then((response) => {
          this.$store.dispatch('updateSketch', this.sketch.id).then(() => {
            this.isLoading = false
          })
    ...
    setPublicAccess: function () {
      this.isLoading = true
      BrowserDB.setSketchPublicAccess(this.sketch.id, this.isPublic)
        .then((response) => {
          this.$store.dispatch('updateSketch', this.sketch.id).then(() => {
            this.isLoading = false
          })

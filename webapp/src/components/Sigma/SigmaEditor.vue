...
export default {
  ...
  methods: {
    ...
    parseSigma: _.debounce(function (ruleYaml) {
      ...
      BrowserDB.getSigmaRuleByText(ruleYaml)
        .then((response) => {
    ...
    getRuleByUUID(ruleUuid) {
      ...
      BrowserDB.getSigmaRuleResource(ruleUuid)
        .then((response) => {
    ...
    deleteRule(ruleUuid) {
      if (confirm('Delete Rule?')) {
        BrowserDB.deleteSigmaRule(ruleUuid)
          .then((response) => {
    ...
    saveRule: function () {
      // Create new rule
      if (!this.ruleId) {
        BrowserDB.createSigmaRule(this.ruleYamlTextArea)
          .then(() => {
            this.$store.dispatch('updateSigmaList')
      ...
      BrowserDB.updateSigmaRule(this.editingRule.id, this.ruleYamlTextArea)
        .then(() => {
          this.$store.dispatch('updateSigmaList')

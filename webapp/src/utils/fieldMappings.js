...
async function getActualDatabaseColumns(sketchId) {
  ...
  try {
    ...
    const searchResult = await BrowserDB.search(sketchId, {
      query: '*',
      filter: { from: 0, size: 1, indices: ['_all'] }
    })
    
    if (searchResult.data.objects && searchResult.data.objects.length > 0) {
      const event = searchResult.data.objects[0]._source

...
class Schema:
    ...
    def validate_schema(self, validation_str):
        ...
        errors = self._validate_helper(
            errors, self.schema, validator["top_level_fields"]
        )
        if errors:
            self.healthy = False
            return errors + ["fatal"]
        ...
        for i, dtype in enumerate(self.schema.get("data_types", [])):
            ...
            for j, file in enumerate(dtype.get("files", [])):
                ...
                if dtype.get("temporal") == "event":
                    errors += self._validate_helper(
                        file,
                        validator["event_file_fields"],
                        f"data_types[{i}].files[{j}]",
                    )
                elif dtype.get("temporal") == "state":
                    errors += self._validate_helper(
                        file,
                        validator["state_file_fields"],
                        f"data_types[{i}].files[{j}]",
                    )

                if "identifiers" in file.keys() and isinstance(
                    file["identifiers"], dict
                ):

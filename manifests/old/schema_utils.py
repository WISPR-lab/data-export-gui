...
def validate_schema(schema_str, validation_str):
    ...
    for i, dtype in enumerate(schema.get("data_types", [])):
        errors += _validate_helper(
            dtype, validator["data_type_fields"], f"data_types[{i}]"
        )
        ...
        for j, file in enumerate(dtype.get("files", [])):
            errors += _validate_helper(
                file, validator["file_fields"], f"data_types[{i}].files[{j}]"
            )
            if dtype.get("temporal") == "event":
                errors += _validate_helper(
                    file, validator["event_file_fields"], f"data_types[{i}].files[{j}]"
                )
            elif dtype.get("temporal") == "state":
                errors += _validate_helper(
                    file, validator["state_file_fields"], f"data_types[{i}].files[{j}]"
                )

            if "identifiers" in file.keys() and isinstance(file["identifiers"], dict):
                ...
                for id, _ in file.get("identifiers", {}).items():
                    if id not in ok_id_fields:
                        errors.append(
                            f"Unknown identifier field {id} in data_types[{i}].files[{j}].identifiers"
                        )

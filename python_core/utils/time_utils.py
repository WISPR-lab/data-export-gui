...
def extract_dates(
    ...
    match_any_pattern = lambda x: any(
        pattern.search(x) for pattern in compiled_patterns
    )
    df["is_time_field"] = df["attribute"].apply(match_any_pattern)

    parse_valid_date = lambda row: (
        parse_date(row["value"], fuzzy, default_tz) if row["is_time_field"] else pd.NA
    )
...
def parse_date(
    ...
    try:
        ...
        if date_str.isdigit():
            ...
            if (
                JAN_1_2000_UNIX <= ts <= JAN_1_2050_UNIX
            ):  # Unix timestamps for 1-1-2000 and 1-1-2050

...
def _get_val(attrs: dict, key: str, preference="first") -> str | None:
    ...
    if preference == "non_generic" and candidates:
        return max(candidates, key=lambda x: (not _generic_model_name(x), _has_version_or_variant(x)))
...
def normalize_device_fields(attrs: dict) -> dict:
    ...
    # now deal with OS
    os_version, inferred_os_name = _decompose_os_version(_get_val(attrs, 'os_version', preference="longest"))

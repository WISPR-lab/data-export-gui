...
def _normalize(rows, platform, ua_parser, table_events=False):
    updates = []
    for row in rows:
...
def normalize(upload_id: str, db_path: str = None) -> dict:
    ...
    with DatabaseSession(db_path, use_dict_factory=True, json_columns=['attributes', 'event_category']) as conn:
        ...
        updates = _normalize(rows, platform, ua_parser, table_events=True)

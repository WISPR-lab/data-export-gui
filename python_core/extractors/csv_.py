...
class CSVParser(BaseParser):
            ...
            for i, record in enumerate(records):
                if i in line_map:
                    ...
                    record["__line_numbers"] = [
                        i + 2
                    ]  # fallback: header on line 1, data starts at line 2
            return records
    ...
    def str_to_df(cls, s: str):
        ...
        def handle_bad_line(line):
            bad_lines.append(
                f"Malformed or ragged line skipped: {','.join(line[:5])}..."
            )
            return None
        ...
        try:
            ...
            raise FileLevelError(
                f"CSV parse failed: {e}", context={"error_type": type(e).__name__}
            )
    ...
    def drop_duplicates(cls, df, dupe_cfg: dict):
        ...
        try:
            ...
            if keep == "row_completeness":
                ...
                df = df.assign(
                    temp_null_count=null_counts, temp_orig_index=range(len(df))
                )
                df = df.sort_values(
                    ["temp_null_count", "temp_orig_index"], ascending=[False, True]
                )
                df = df.drop_duplicates(subset=subset, keep="first")
                df = df.sort_values("temp_orig_index").drop(
                    columns=["temp_null_count", "temp_orig_index"]
                )
            else:

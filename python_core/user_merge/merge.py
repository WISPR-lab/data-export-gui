...
def merge_device_profiles(conn, src_profile_id: str, tgt_profile_id: str) -> dict:
    try:
        ...
        if src_spec >= 3 or tgt_spec >= 3:
            return {'status': 'ineligible', 'message': 'Cannot merge hard-merged profiles'}
        
        if src_is_generic and tgt_is_generic:
            return {'status': 'ineligible', 'message': 'Cannot merge two generic profiles'}
        ...
        conn.execute(
            '''UPDATE device_profiles 
               ...
               WHERE id = ?''',
            (
                str(merged_profile['atomic_devices_ids']),
                merged_profile['attributes'],
                merged_profile['specificity'],
                merged_profile['model'],
                merged_profile['manufacturer'],
                merged_profile['origins'],

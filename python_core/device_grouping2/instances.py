...
class DeviceInstance:
    ...
    def __init__(self, root_id: str, df: pd.DataFrame):
        ...
        if not valid_ts.empty:
            ...
            max_ts = valid_ts.max()
            self.first_seen = (
                float(min_ts.timestamp())
                if hasattr(min_ts, "timestamp")
                else float(min_ts)
            )
            self.last_seen = (
                float(max_ts.timestamp())
        ...
        self.os_versions = (
            ...
            if "attr__norm__os_version" in self.df.columns
            else []
        )
        self.client_versions = (
            self.df["attr__norm__client_version"].dropna().unique().tolist()
            if "attr__norm__client_version" in self.df.columns
            else []
        )
        self.client_ips = (
            self.df["attr__client_ip"].dropna().unique().tolist()
            if "attr__client_ip" in self.df.columns
            else []
        )
        self.locations = (
            self.df["attr__location"].dropna().unique().tolist()
            if "attr__location" in self.df.columns
            else []
        )
    ...
    def _evaluate_apple_masking(self) -> Optional[int]:
        ...
        client_val = (
            (self._find_best_attribute("attr__norm__client_name") or "").strip().lower()
        )
        model_val = (
            (self._find_best_attribute("attr__norm__model_name") or "").strip().lower()
        )
        ...
        is_generic_ios = os_val in ("ios", "iphone os") and model_val in (
            "iphone",
            "ipad",
            "ipod",
        )
    ...
    def export_as_dict(self) -> dict:
        ...
        return {
            ...
            "latest_client_version": self.client_versions[-1]
...
class DeviceInstanceGraph:
    ...
    @staticmethod
    def format_initial(
        events_df: pd.DataFrame, devices_df: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        if events_df.empty:
            events_df = pd.DataFrame(
                columns=["id", "upload_id", "attributes", "origin", "timestamp"]
            )
        events_df["table"] = "events"
        events_df["timestamp"] = pd.to_datetime(
            events_df["timestamp"], unit="ms", errors="coerce"
        )

        if devices_df.empty:
            devices_df = pd.DataFrame(
                columns=["id", "upload_id", "attributes", "origin"]
            )
        ...
        parsed_json = df["attributes"].apply(
            lambda x: (
                json.loads(x)

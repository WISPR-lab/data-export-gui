...
def _pass1_client(events_df: pd.DataFrame, max_days = MAX_DAYS_CLIENT_DIFF) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    no_id_match = df[keys].fillna("").ne(df[keys].fillna("").shift()).any(axis=1).tolist()
...
def _pass2_os(subgraph_df: pd.DataFrame, max_days=MAX_DAYS_OS_DIFF) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    edges = pairs[['last_node_id_F', 'first_node_id_G', 'os_version_F', 'os_version_G']].rename(
...
def get_edges(df: pd.DataFrame, run_pass2: bool = None) -> pd.DataFrame:
    ...
    if run_pass2 is None:
        from db_session import get_config_value

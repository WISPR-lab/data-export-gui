...
def group(upload_id: str, db_path: str = None, conn=None) -> None:
    ...
    with DatabaseSession(
        ...
        raw_rows = conn.execute(
            "SELECT id, upload_id, entity_type, entity_sub_type, origin, attributes, raw_data_id FROM devices_raw WHERE upload_id = ?",
            (upload_id,),
        ).fetchall()
        ...
        if resolved_sessions_registration_rows:
            conn.executemany(
                ...
                INSERT INTO resolved_sessions_registrations (
                    ...
                    os_name, os_version, os_type, attributes, raw_data_ids, is_reduced_ua,
